# Hi, I'm Shiien (모시은) 👋

## :computer: TECH STACK
<div align="left">

  <!-- Language / Runtime -->
  <img src="https://img.shields.io/badge/Java-265A8F.svg?style=flat-square&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB.svg?style=flat-square&logo=python&logoColor=white" />
</div>
  
<div align="left">
  <!-- Spring & Back-end -->
  <img src="https://img.shields.io/badge/Spring%20Boot-6DB33F.svg?style=flat-square&logo=springboot&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring%20Security-6DB33F.svg?style=flat-square&logo=springsecurity&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring%20Cloud-6DB33F.svg?style=flat-square&logo=spring&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring%20Cloud%20Gateway-6DB33F.svg?style=flat-square&logo=spring&logoColor=white" />
  <img src="https://img.shields.io/badge/Eureka-6DB33F.svg?style=flat-square&logo=spring&logoColor=white" />
</div>

<div align="left">
  <!-- Messaging / Event -->
  <img src="https://img.shields.io/badge/Apache%20Kafka-231F20.svg?style=flat-square&logo=apache-kafka&logoColor=white" />
</div>

<div align="left">
  <!-- DB / Cache -->
  <img src="https://img.shields.io/badge/PostgreSQL-4169E1.svg?style=flat-square&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/MongoDB-47A248.svg?style=flat-square&logo=mongodb&logoColor=white" />
  <img src="https://img.shields.io/badge/Redis-DC382D.svg?style=flat-square&logo=redis&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1.svg?style=flat-square&logo=mysql&logoColor=white" />
</div>

<div align="left"> 
  <!-- DevOps / Cloud -->
  <img src="https://img.shields.io/badge/Docker-2496ED.svg?style=flat-square&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Kubernetes-326CE5.svg?style=flat-square&logo=kubernetes&logoColor=white" />
  <img src="https://img.shields.io/badge/AWS-232F3E.svg?style=flat-square&logo=amazon-aws&logoColor=white" />
</div>


<div align="left">
  <!-- Docs / Test / Build -->
  <img src="https://img.shields.io/badge/Swagger-85EA2D.svg?style=flat-square&logo=swagger&logoColor=black" />
  <img src="https://img.shields.io/badge/JUnit5-25A162.svg?style=flat-square&logo=junit5&logoColor=white" />
  <img src="https://img.shields.io/badge/Gradle-02303A.svg?style=flat-square&logo=gradle&logoColor=white" />

</div>

<!-- Data / Py stack -->
<div align="left">
  <img src="https://img.shields.io/badge/pandas-150458.svg?style=flat-square&logo=pandas&logoColor=white" />
  <img src="https://img.shields.io/badge/numpy-4D77CF.svg?style=flat-square&logo=numpy&logoColor=white" />
  <img src="https://img.shields.io/badge/Matplotlib-11557C.svg?style=flat-square&logo=Matplotlib&logoColor=white" />
</div>

## :hammer: TOOLS
<div align="left">
  <img src="https://img.shields.io/badge/GitHub-181717.svg?style=flat-square&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/Notion-000000.svg?style=flat-square&logo=notion&logoColor=white" />
  <img src="https://img.shields.io/badge/IntelliJ%20IDEA-000000.svg?style=flat-square&logo=intellijidea&logoColor=white" />
  <img src="https://img.shields.io/badge/Jupyter-F37626.svg?style=flat-square&logo=jupyter&logoColor=white" />
  <img src="https://img.shields.io/badge/Postman-FF6C37.svg?style=flat-square&logo=postman&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub%20Actions-2088FF.svg?style=flat-square&logo=githubactions&logoColor=white" />
</div>



## 🧑‍🎓 PROJECTS

### 1) People of Delivery
#### Phase 3 (Event-Driven & Cloud-Native)
- **Repo:** https://github.com/goorm-engineers4/Project3_MSA_people_of_delivery  
- **Highlights:** Kafka 기반 EDA, Outbox + Saga 보상 트랜잭션, Redis+Lua 동시성 제어, 내부 Passport 토큰, K8s/Helm/Argo Rollouts, Prometheus/Grafana/OTel 관측성
- **Stack:** Java 21 · Spring Boot · JPA · Spring Security · Kafka · PostgreSQL · Redis · MongoDB · Kubernetes · Docker · Swagger/OpenAPI

#### Phase 2 (MSA Edition)
- **Repo:** https://github.com/goorm-engineers4/MSA_people_of_delivery  
- **Highlights:** 모놀리식 → MSA 분해, Spring Cloud Gateway + Eureka, 서비스별 DB 분리, JWT(Access/Refresh) + Redis, PostgreSQL→Mongo 실시간 동기화, Toss 결제 연동, GitHub Actions + AWS OIDC + ECS Blue/Green 배포
- **Stack:** Java 21 · Spring Boot · JPA · Spring Security · PostgreSQL · MongoDB · Redis · Kafka(초기) · Docker/Compose · GitHub Actions · Swagger/OpenAPI

### 2) AI 자동화 Sales CRM
- **Repo:** https://github.com/aivle12/sAIes_CRM  
- **One-liner:** 영업 리드·기회·주문/상품을 **Spring MVC + Mustache(SSR)** 로 구현한 학습형 CRM.
- **Highlights:**
  - Mustache 기반 서버사이드 렌더링(templates) + Spring MVC 라우팅, 레이어드 구조(Entity→Repository→Service→Controller→View)
  - CRM 도메인(Leads/Opportunities/Orders/Products) 중심의 CRUD 설계와 DTO 분리
  - Gradle 빌드 & Dockerfile로 로컬 실행 표준화
- **Stack:** Java · Spring Boot · Spring MVC · Mustache · PostgreSQL · Gradle · Docker



## 🏅 STATS
<div align="left">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=shiien14&layout=compact&bg_color=180,00000000,&title_color=000000&text_color=000000"/><br/> 
  <a href="https://git.io/streak-stats"><img src="https://streak-stats.demolab.com?user=shiien14" alt="GitHub Streak" /></a>
</div>



## :sunglasses: ALGORITHM
[![Solved.ac프로필](http://mazassumnida.wtf/api/v2/generate_badge?boj=inin9090)](https://solved.ac/inin9090)
